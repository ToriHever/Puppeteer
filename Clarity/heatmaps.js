...
(async () => {
  ...
  const csvWriter = createObjectCsvWriter({
    ...
    header: [
      { id: 'url', title: 'URL' },
      { id: 'pctScroll', title: '% прокрутки' },
      { id: 'visitors', title: '# посетителей' },
      { id: 'pctLeave', title: '% уходит' }
    ],
  ...
  async function processUrl(url) {
    ...
    try {
      // 2) Навигация
      await page.goto(
        'https://clarity.microsoft.com/projects/view/fumb9hruw3/heatmaps?date=Last%2030%20days',
        { waitUntil: 'networkidle2', timeout: 30000 }
      );
      console.log('  ✔ Heatmaps открыт');

      // 4–6) Поиск URL
      await page.waitForSelector('#SuggestionSearchBoxheatmaps0zero', { timeout: 10000 });
      await page.click('#SuggestionSearchBoxheatmaps0zero');
      await page.type('#SuggestionSearchBoxheatmaps0zero', url, { delay: 50 });
      ...
      console.log(`  ✔ Parsed ${csvRecords.length} rows`);
            ...
            // 10) Запись в CSV
      const recordsToWrite = csvRecords.map(rec => ({ url, ...rec }));
      await csvWriter.writeRecords(recordsToWrite);
      console.log(`  ✔ Записано ${recordsToWrite.length} строк в CSV`);('  ✔ Все записи сохранены');
