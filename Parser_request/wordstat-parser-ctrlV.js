...
import {
    sendTelegramMessage
} from '../Notifications_Telegram.js';
...
function promptCommand(rl) {
    return new Promise(res => rl.question('> ', ans => res(ans.trim())));
}
    ...
    try {
        ...
        const csvWriter = createCsvWriter({
            ...
            header: [
                ...
                {
                    id: 'frequencyWithQuotes',
                    title: 'Частота с кавычками'
                },
                {
                    id: 'frequencyWithExclamation',
                    title: 'Частота с восклицаниями'
                }
      ],
        ...
        while (true) {
            ...
            if (cmd === 'login') {
                ...
                if (page.url().startsWith(TARGET_URL)) {
                    ...
                    break;
                } else console.log(`Неверная страница: ${page.url()}. Откройте ${TARGET_URL}`);
            } else console.log('Используйте: login, save-cookie или run');
        }
        rl.close();
        ...
        for (const {
            ...
            while (!processed) {
                try {
                    ...
                    await page.keyboard.press('Backspace'); // 1) Сбрасываем текущее значение инпута
                    await delay(getRandomDelay(500, 1000)); // 2) Небольшая пауза, чтобы страница успела "отреагировать"
                    await page.type('.textinput__control', query);// 3) Печатаем запрос прямо в поле (без буфера обмена)
                    await page.keyboard.press('Enter');  // 4) Отправляем форму (Enter)
